				...
				else
				{
					...
					if (!haveflowery)
					{
						if (wwho == 2 && global.weapon[global.submenucoord[global.submenu]] == 51 && scr_havechar("ralsei") && global.submenu == 12)
							scr_itemcomment(2, stringsetloc("Don't write that on it!!", "obj_darkcontroller_slash_Step_0_gml_1379_0"))
					}
				}
...
{
	drawchar = 1
	bpy = 60 - (20 * array_get(scr_platswap_yscale(), 0) * (global.interact != 5))
	tpy = 80
	if (global.interact == 5)
	{
		if (tp < (tpy - 1))
