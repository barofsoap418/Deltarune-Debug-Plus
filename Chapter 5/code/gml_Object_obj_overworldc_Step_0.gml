	...
	if (button1_p())
	{
		if (global.menuno == 5)
		{
			...
			if (global.menucoord[5] == 2)
			{
				...
				if (global.litem[global.menucoord[1]] == 21)
				{
					dontthrow = 1
					dontthrowtype = 3
				}
				var is_weapon_type = is_lw_item_weapon_type(global.litem[global.menucoord[1]])
				if (is_weapon_type)
				{
					dontthrow = 1
					dontthrowtype = 5
				...
				if (dontthrow == 0)
				{
					...
					if (global.litem[global.menucoord[1]] == 19)
						scr_flag_set(1710, 1)
					script_execute(scr_writetext, 0, "x", 0, 0)
				...
				if (dontthrow == 1)
				{
					...
					{
						msgsetloc(0, "* (For some reason you felt like if you throw it away...)/", "obj_overworldc_slash_Step_0_gml_77_0")
						msgnextloc("* (It would be like throwing away someone's..^1. ???)/", "obj_overworldc_slash_Step_0_gml_78_0")
						msgnextloc("* (..^1. but you didn't fully understand it.)/%", "obj_overworldc_slash_Step_0_gml_79_0_b")
						script_execute(scr_writetext, 0, "x", 0, 0)
					}
					else if (dontthrowtype == 3)
					{
						scr_speaker("no_name")
						msgsetloc(0, "* (The seeds stick to everything and cannot be thrown away.)/%", "obj_overworldc_slash_Step_0_gml_85_0")
						script_execute(scr_writetext, 0, "x", 0, 0)
					}
					else if (dontthrowtype == 4)
					{
						scr_speaker("no_name")
						if (global.litem[global.menucoord[1]] == 20)
							msgsetloc(0, "* (Couldn't.)/%", "obj_overworldc_slash_Step_0_gml_101_0")
						else
							scr_text(1455)
						script_execute(scr_writetext, 0, "x", 0, 0)
					}
					else if (dontthrowtype == 5)
					{
						msgsetloc(0, "* (Recently, seems like weapons can't be thrown away so easily.)/%", "obj_overworldc_slash_Step_0_gml_107_0")
						script_execute(scr_writetext, 0, "x", 0, 0)
					}
				}
			}
		...
		if (global.menuno == 3)
		{
			...
			else if (global.plot >= 550)
			{
				if (!i_ex(obj_ch5_LWF_town_cell))
					instance_create(0, 0, obj_ch5_LWF_town_cell)
				with (obj_ch5_LWF_town_cell)
					call()
				global.menuno = -1
			}
			else
