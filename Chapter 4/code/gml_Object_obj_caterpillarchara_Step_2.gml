if (scr_debug())
{
	if (!keyboard_check(vk_control) && keyboard_check(vk_shift) && keyboard_check_pressed(ord("N"))) // i had no idea this was here but it overlaps with the noclip command so i added an extra check for the ctrl key not being held down
	{
		target--
		debug_print("name:" + name + " | target=" + string(target))
	}
}
if (!init)
{
