...
if (global.interact == 5)
{
	...
	if (button1_p())
	{
		if (global.menuno == 5)
		{
			...
			if (global.menucoord[5] == 2)
			{
				...
				if (dontthrow == 1)
				{
					...
					{
						msgsetloc(0, "* (For some reason you felt like if you throw it away...)/", "obj_overworldc_slash_Step_0_gml_79_0")
						msgnextloc("* (It would be like throwing away someone's..^1. ???)/", "obj_overworldc_slash_Step_0_gml_80_0")
						msgnextloc("* (..^1. But you didn't fully understand it.)/%", "obj_overworldc_slash_Step_0_gml_81_0")
						script_execute(scr_writetext, 0, "x", 0, 0)
					}
					else if (dontthrowtype == 3)
					{
						msgsetloc(0, "* (Recently, seems like weapons can't be thrown away so easily.)/%", "obj_overworldc_slash_Step_0_gml_86_0")
						script_execute(scr_writetext, 0, "x", 0, 0)
					}
				}
			}
		...
		if (global.menuno == 3)
		{
			...
			{
				scr_speaker("no_name")
				msgsetloc(0, "* (You checked your phone's contacts and recent dials.)/", "obj_overworldc_slash_Step_0_gml_113_0")
				msgnextloc("* (..^1. but everything has been deleted.)/%", "obj_overworldc_slash_Step_0_gml_114_0")
				d_make()
				global.menuno = 9
			}
			else
