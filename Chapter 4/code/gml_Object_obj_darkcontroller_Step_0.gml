			...
			if (button1_p() && onebuffer < 0)
			{
				...
				if (canequip == 1)
				{
					...
					if (global.submenu == 13 || global.submenu == 14)
					{
						...
						scr_armorinfo_mine()
						scr_armorinfo_all()
						if (wwho == 3 && newequip == 50 && scr_havechar("susie"))
							scr_itemcomment(1, stringsetloc("(Too bad)", "obj_darkcontroller_slash_Step_0_gml_1297_0"))
						if (wwho == 3 && newequip == 26)
						{
							if (global.flag[1037] < 15)
								global.flag[1037]++
				...
				{
					snd_play(snd_cantselect)
					if (wwho == 2 && global.weapon[global.submenucoord[global.submenu]] == 51 && scr_havechar("ralsei") && global.submenu == 12)
						scr_itemcomment(2, stringsetloc("Don't write that on it!!", "obj_darkcontroller_slash_Step_0_gml_1328_0"))
				}
