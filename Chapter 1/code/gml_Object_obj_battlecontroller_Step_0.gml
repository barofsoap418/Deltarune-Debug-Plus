if (victory == 1 && victoried == 0)
{
	...
	if (skipvictory == 0)
	{
		...
		global.fc = 0
		global.fe = 0
		global.battlemsg[0] = scr_84_get_subst_string(scr_84_get_lang_string("obj_battlecontroller_slash_Step_0_gml_40_0"), string(global.monsterexp[3]), string(global.monstergold[3])) // * You won^1!&* Got ~1 EXP and ~2 D$./%
...
if (scr_debug())
{
    if (keyboard_check_pressed(ord("V")) && global.turntimer > 0 && instance_exists(obj_growtangle) && scr_isphase("bullets"))
    {
        global.turntimer = 0
    ...
    if (keyboard_check_pressed(vk_f6))
    {
        for (var i = 0; i < 3; i++)
        {
            ...
            {
                with (instance_create(x + (sprite_width / 2), y + (sprite_height / 2), obj_animation_dx))
                {
